...
import { loadItemDetail } from "@/lib/queries";

export const dynamic = "force-dynamic";

const MATERIAL_RE = /^[A-Za-z0-9_]{1,64}$/;

/** Zeitraum-Optionen für den Graphen (Tage; null = alles). */
...
export async function GET(
  ...
  try {
    const detail = await loadItemDetail(material.toUpperCase(), days);
    return NextResponse.json({ ok: true, ...detail });
  } catch (e) {
    console.error("[api/items/detail]", e);
