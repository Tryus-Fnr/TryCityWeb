...
import { loadClusterMetrics, parseRange } from "@/lib/infra";
...
export async function GET(request: NextRequest) {
  ...
  try {
    const metrics = await loadClusterMetrics(range);
    return NextResponse.json({ ok: true, metrics, range, at: Date.now() });
  } catch (e) {
